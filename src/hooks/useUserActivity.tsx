...
export function useUserActivity(userId: string | undefined) {
  ...
  const logUserActivity = async (actionType: string, lawName?: string, articleNumber?: string) => {
    ...
    try {
      ...
      // Primeiro, obtemos os pontos atuais
      ...
      // Atualizamos com os novos valores
      ...
      // Atualizamos os pontos de atividade também
      const { error: activityError } = await supabase
        .from('user_profiles')
        .update({ 
          activity_points: (userData?.activity_points || 0) + 10 
        })
        .eq('id', userId);
